...
end_point['geometry'] = end_point[['shape_pt_lat', 'shape_pt_lon']].apply(
...
station['geometry'] = station[['stop_lat', 'stop_lon']].apply(
    lambda x: Point(x[0], x[1]), axis=1)
...
finaltable = allstation.merge(tmp, how='left', on=['shape_id', 'shape_group']).dropna()[
    ['shape_id', 'shape_pt_sequence', 'shape_group', 'speed/mph']]

shapeline = {}
for i in set(finaltable.shape_id):
    for j in set(finaltable[finaltable['shape_id'] == i]['shape_group']):
        tmpdict = {}
        line = LineString(zip(finaltable['shape_pt_lon'][(finaltable['shape_group'] == j) & (finaltable[
                          'shape_id'] == i)], finaltable['shape_pt_lat'][(finaltable['shape_group'] == j) & (finaltable['shape_id'] == i)]))
        tmpdict['geometry'] = line
        tmpdict['speed'] = max(finaltable[
                               'speed/mph'][(finaltable['shape_group'] == j) & (finaltable['shape_id'] == i)])
        shapeline[str(i) + '-' + str(int(j))] = tmpdict

with open('speed_shape.json', 'w') as outfile:
    json.dump(json.dumps(df(shapeline).transpose().to_json()), outfile)
