...
L.UtfGrid = (L.Layer || L.Class).extend({
	...
	_update: function () {
        ...
        if (this.options.bounds) {
            var ne = this._map.project(this.options.bounds._northEast);
            var sw = this._map.project(this.options.bounds._southWest);
            if (bounds.min.x<sw.x) bounds.min.x = sw.x;
            if (bounds.min.y<ne.y) bounds.min.y = ne.y;
            if (bounds.max.x>ne.x) bounds.max.x = ne.x;
            if (bounds.max.y>sw.y) bounds.max.y = sw.y;
        }
